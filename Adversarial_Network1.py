...
from keras.optimizers import Adadelta, Nadam, RMSprop, SGD, Adam
...
class GenModel:
    ...
    def get_defined_input_model(self, shape):
        ...
        model1 = Model(inputs=[chnl4_input, chnl3_input], outputs=[out])
...
class AdModel:
    def __init__(self, batch_size=32, disc_loss_weight=0.5):
    ...
    def load_gen_model(self, path=r'C:\Users\buggyr\Mosaic_Experiments\models\2018-02-07 20-22_UNET_2_layer_64x64_logcosh_normal1_Patterns+Gharbi_2_input_strides+upsample\Epoch_Models\model.111-0.00.hdf5'):

        gmodel = GenModel()
        model = gmodel.load_model(path)
        weights = model.get_weights()

        self.gen_model = gmodel.get_defined_input_model(shape=(128, 128, 3))
        self.gen_model.set_weights(weights)
    ...
    def load_disc_model(self,
        ...
        dmodel = DiscModel()
        self.disc_model = dmodel.load_model(path)
    ...
    def assm_ad_model(self):
        ...
        self.ad_model = Model(inputs=self.gen_model.inputs, outputs=[self.gen_model.output, self.disc_model(self.gen_model.output)])
    ...
    def train(self, load_training, batch_size=32, epochs=50):
        ...
        self.gen_model.compile(optimizer=optimizer_func, loss=self.gen_loss_func)
        self.disc_model.compile(optimizer=optimizer_func, loss=self.disc_loss_func)
        self.ad_model.compile(optimizer=optimizer_func, loss=[self.gen_loss_func, self.disc_loss_func], loss_weights=[1 - self.disc_loss_weight, self.disc_loss_weight])
        ...
        for ep in range(epochs):
            ...
            for i in range(fls):
                train_generator = train_generator_rgb_tiled(load_training, 64, batch_size)
                ...
                gen_batch = self.gen_model.predict_on_batch(inputs)

                target_label = [0] * batch_size + [1] * batch_size
...
class DiscModel:
    ...
    def build_model(self):
        ...
        conv1 = AveragePooling2D(pool_size=(2, 2))(conv1)

        conv2 = Conv2D(32, (3, 3), activation='relu', padding='same')(conv1)

        conv2 = AveragePooling2D(pool_size=(2, 2))(conv2)
...
if __name__ == "__main__":
    ...
    adm.train(load_training=load_training, batch_size=32)
